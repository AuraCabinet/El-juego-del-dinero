...
function init() {
  ...
  document.body.dataset.routeLocked = state.playerName ? "true" : "false";
  initFirebase();
  setupOnboarding();
  ...
  document.getElementById("name-form").addEventListener("submit", event => {
    ...
    state.lockedLevel = selectedLevel;
    document.body.dataset.level = selectedLevel;
    document.body.dataset.routeLocked = "true";
  ...
  document.querySelectorAll(".levels button[data-level]").forEach(button => {
    button.onclick = () => {
      ...
      if (state.playerName && +button.dataset.level !== locked) {
  ...
  if (state.playerName) {
    selectedLevel = state.lockedLevel || state.level || selectedLevel;
    document.body.dataset.level = selectedLevel;
    document.body.dataset.routeLocked = "true";
    modules = buildModules(selectedLevel);
    showGame();
...
function renderRouteLock(message = "") {
  ...
  box.innerHTML = `<div><b>Ruta activa: ${routeLabel()}</b><span>${message || "Esta ruta se eligió al entrar y las demás quedan cerradas para mantener la progresión correcta."}</span></div><button id="route-family-open" type="button">${familyTitle}</button><small>${familyText}</small>`;
  ...
  document.querySelectorAll(".levels button[data-level]").forEach(button => {
    const locked = state.lockedLevel || selectedLevel;
    button.disabled = +button.dataset.level !== locked;
    button.title = button.disabled ? "Ruta cerrada para este estudiante" : "Ruta activa";
  });
}
