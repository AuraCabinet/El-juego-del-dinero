...
function scenarioFor(level, world, zone, topicIndex, activityIndex) {
  ...
  const endings = {
    1: ["Mira bien antes de elegir.", "Cuenta despacio y pregunta si dudas.", "Piensa qué necesitas y qué puedes esperar."],
    2: ["Revisa el precio final antes de pagar.", "No decidas solo por presión.", "Mira saldo, seguridad y meta."],
    3: ["Revisa monto, plazo y reserva.", "Piensa en el efecto de esta decisión.", "Compara antes de comprometer dinero."],
    4: ["Revisa caja, costo y margen.", "Piensa en cliente, cobro y riesgo.", "Decide con datos antes de crecer."]
  };
  return `${base}. ${endings[level][(topicIndex + activityIndex) % endings[level].length]}`;
}
...
function questionText(level, key, topic, nums, place, scenario, world, zone) {
  ...
  const child = {
    choose: `Tienes dinero para comprar algo pequeño. ¿Qué haces primero?`,
    true: `¿Qué frase explica mejor ${lower}?`,
    order: `¿Cuál es el orden correcto antes de comprar?`,
    match: `¿Cuál ejemplo se parece más a ${lower}?`,
    classify: `¿Cómo clasificas esta compra?`,
    calc: `Tienes $${nums[0]} y guardas $${nums[1]}. ¿Cuánto queda para usar?`,
    fill: `${clean} me ayuda a...`,
    risk: `¿Cuándo debes parar y pedir ayuda?`,
    express: `Reto rápido: antes de comprar, ¿qué haces?`,
    compare: `Ves dos opciones. ¿Cuál cuida mejor tu meta?`,
    plan: `¿Cuál es un buen mini plan para esta semana?`,
    explain: `¿Cómo lo explicarías con palabras fáciles?`,
    memory: `¿Qué regla corta te ayuda a decidir?`,
    simulate: `Si compras sin mirar, ¿qué puede pasar?`,
    compound: compoundQuestion(level, nums, clean)
  };
  if (level === 1) return child[key];
  const teen = {
    choose: `Vas a decidir sobre ${lower}. ¿Qué opción te da más control?`,
    true: `¿Qué frase explica ${lower} de forma clara?`,
    order: `¿Cuál orden usarías antes de pagar o guardar?`,
    match: `¿Qué consecuencia se relaciona mejor con ${lower}?`,
    classify: `¿Qué acción cuida mejor tu mesada, seguridad y futuro?`,
    calc: `Recibes $${nums[0]} y separas $${nums[1]}. ¿Cuánto queda disponible?`,
    fill: `${clean} sirve para...`,
    risk: `¿Qué señal muestra riesgo antes de pagar?`,
    express: `Revisión rápida: ¿qué haces antes de decidir?`,
    compare: `¿Qué opción conviene al comparar precio final, uso y meta?`,
    plan: `¿Cuál plan de 7 días tiene más sentido?`,
    explain: `¿Qué explicación demuestra que entendiste ${lower}?`,
    memory: `¿Qué regla ayuda a no caer por impulso o presión?`,
    simulate: `Si ignoras costo, plazo o seguridad, ¿qué puede pasar?`,
    compound: compoundQuestion(level, nums, clean)
  };
  if (level === 2) return teen[key];
  const adult = {
    choose: `En una decisión sobre ${lower}, ¿qué opción protege mejor tu reserva?`,
    true: `¿Qué definición práctica sirve para decidir?`,
    order: `¿Cuál es el proceso responsable antes de comprometer dinero?`,
    match: `¿Qué aplicación real corresponde a ${lower}?`,
    classify: `¿Qué acción protege liquidez, riesgo y patrimonio?`,
    calc: `Entran $${nums[0]} y reservas $${nums[1]}. ¿Cuánto queda para operar?`,
    fill: `${clean} convierte una idea en...`,
    risk: `¿Cuál es el riesgo principal antes de avanzar?`,
    express: `Revisión mínima responsable: ¿qué miras primero?`,
    compare: `¿Qué opción equilibra costo total, beneficio, plazo y consecuencia?`,
    plan: `¿Qué plan incluye monto, fecha, responsable y revisión?`,
    explain: `¿Qué explicación defendería mejor la decisión?`,
    memory: `¿Qué regla financiera es más sólida?`,
    simulate: `¿Qué pasa si decides sin datos suficientes?`,
    compound: compoundQuestion(level, nums, clean)
  };
  if (level === 3) return adult[key];
  return {
    choose: `En ${lower}, ¿qué decisión protege caja, margen y control?`,
    true: `¿Qué afirmación sirve para dirigir el negocio?`,
    order: `¿Cuál es la secuencia correcta antes de crecer?`,
    match: `¿Qué indicador o consecuencia corresponde a ${lower}?`,
    classify: `¿Qué acción cuida caja, cliente, riesgo y escala?`,
    calc: `Entran $${nums[0]} y reservas $${nums[1]}. ¿Cuánto queda para operar?`,
    fill: `${clean} ayuda a gestionar...`,
    risk: `¿Qué riesgo puede romper caja o reputación?`,
    express: `Revisión de dirección: ¿qué miras antes de avanzar?`,
    compare: `¿Qué opción equilibra margen, cobro, riesgo y aprendizaje?`,
    plan: `¿Qué plan incluye meta, indicador, responsable y fecha?`,
    explain: `¿Qué explicación convencería a un equipo serio?`,
    memory: `¿Qué regla evita crecer a ciegas?`,
    simulate: `¿Qué pasa si vendes sin caja, contrato o margen?`,
    compound: compoundQuestion(level, nums, clean)
  }[key];
}

function compoundQuestion(level, nums, topic) {
  if (level === 1) return `Si guardas monedas varias semanas, ¿qué puede pasar con tu meta?`;
  if (level === 2) return `Si separas $${nums[1]} cada semana y no lo gastas, ¿qué idea estás practicando?`;
  if (level === 3) return `¿Qué resume mejor el interés compuesto?`;
  return `¿Cómo usarías reinversión e interés compuesto sin perder control del negocio?`;
}
...
function correctText(level, key, topic, nums, place, scenario, world, zone) {
  ...
  const childAnswers = {
    choose: `Parar, mirar el precio y pedir ayuda si dudas`,
    true: `El dinero se cuenta y se cuida antes de gastarlo`,
    order: "Mirar el precio, contar monedas, preguntar y decidir",
    match: `${topicClean(topic)} = algo que veo en casa, la tienda o el colegio`,
    classify: `Necesidad primero; deseo después si alcanza`,
    calc: `$${left}`,
    fill: `cuidar lo que tengo y decidir mejor`,
    risk: `Comprar sin mirar si alcanza para lo importante`,
  ...
  const teenAnswers = {
    choose: `Revisar saldo, costo real y consecuencia antes de decidir`,
...
function wrongOptions(level, key, topic, nums, place, scenario, world, zone) {
  ...
  const typed = {
    ...
    match: [
      `Comprar sin revisar`,
      `Usar todo hoy`,
      `Ignorar precio, plazo y seguridad`
    ],
    classify: [
      `Deseo urgente aunque rompa la meta`,
  ...
  if (key === "true") return trueWrongOptions(level, topic);
...
function trueWrongOptions(level, topic) {
  const focus = focusFor(topic);
  if (level === 1) {
    if (focus === "seguridad") return ["Una clave se puede contar a cualquiera", "Si un enlace brilla, siempre es seguro", "No hace falta pedir permiso con una tarjeta"];
    if (focus === "liquidez") return ["El precio no importa si algo gusta mucho", "Gastar todo hoy siempre es mejor", "No hace falta contar el cambio"];
    if (focus === "crecimiento") return ["La alcancía crece más si la vacías cada día", "Ahorrar no sirve para cumplir metas", "Esperar nunca ayuda"];
    return ["Decidir con prisa siempre ayuda", "Comprar sin mirar es una buena idea", "No preguntar nunca es mejor"];
  }
  if (level === 2) {
    if (focus === "seguridad") return ["Cualquier enlace sirve para pagar", "Compartir claves no tiene riesgo", "Los datos personales no importan"];
    if (focus === "liquidez") return ["El pago final no hace falta revisarlo", "Una suscripción pequeña nunca afecta", "La mesada alcanza aunque no la mires"];
    return ["La presión del grupo decide mejor que tú", "Comprar por impulso siempre sale bien", "No comparar ahorra dinero"];
  }
  if (level === 3) {
    if (focus === "seguridad") return ["El banco siempre pide claves por enlace", "Un contrato no necesita leerse", "La protección de datos es opcional"];
    if (focus === "liquidez") return ["La reserva se usa primero para deseos", "La cuota baja siempre es la mejor", "No hace falta mirar el costo total"];
    return ["Invertir todo en una sola opción reduce el riesgo", "La deuda no necesita plazo claro", "El presupuesto solo sirve si sobra dinero"];
  }
  if (focus === "seguridad") return ["Los datos de clientes pueden guardarse sin cuidado", "Cualquier contrato verbal basta", "La ciberseguridad no afecta al negocio"];
  if (focus === "liquidez") return ["Vender mucho siempre significa tener caja", "El margen no importa si hay clientes", "Mezclar caja personal y negocio ayuda"];
  return ["Crecer sin medir siempre es mejor", "El proveedor decide tu estrategia", "La reputación no afecta las ventas"];
}
...
function init() {
  ...
  document.body.dataset.routeLocked = state.playerName && !state.isAdmin ? "true" : "false";
  document.body.dataset.admin = state.isAdmin ? "true" : "false";
  initFirebase();
  setupOnboarding();
  ...
  document.getElementById("name-form").addEventListener("submit", event => {
    ...
    state.lockedLevel = isAdminLogin ? null : selectedLevel;
    document.body.dataset.level = selectedLevel;
    document.body.dataset.routeLocked = isAdminLogin ? "false" : "true";
    document.body.dataset.admin = isAdminLogin ? "true" : "false";
  ...
  document.querySelectorAll(".levels button[data-level]").forEach(button => {
    button.onclick = () => {
      const locked = state.lockedLevel || selectedLevel;
      if (state.playerName && !state.isAdmin && +button.dataset.level !== locked) {
        renderRouteLock("Esa ruta está cerrada para este estudiante. Para cambiar de edad, crea una nueva entrada.");
        return;
      }
  ...
  document.getElementById("change-entry").onclick = () => backToEntry();
  setupAvatarInteractivity();
  if (state.playerName) {
    selectedLevel = state.isAdmin ? (state.level || selectedLevel) : (state.lockedLevel || state.level || selectedLevel);
    document.body.dataset.level = selectedLevel;
    document.body.dataset.routeLocked = state.isAdmin ? "false" : "true";
    document.body.dataset.admin = state.isAdmin ? "true" : "false";
    modules = buildModules(selectedLevel);
    showGame();
  }
...
function backToEntry() {
  ...
  safeCloseDialog("mission");
  safeCloseDialog("certificate");
  state.playerName = "";
  state.isAdmin = false;
  state.lockedLevel = null;
  document.body.dataset.routeLocked = "false";
...
function renderRouteLock(message = "") {
  const box = document.getElementById("route-lock");
  if (!box) return;
  const familyTitle = familyGameTitle();
  const familyText = isChildFamilyMode()
    ? "Tienes acceso a El Tesoro de la Familia, el tablero infantil por turnos."
    : "Tienes acceso a El Rico de la Familia, el tablero competitivo familiar.";
  const adminText = "Modo administradora activo: puedes revisar todas las rutas, mundos, juegos familiares y subruta Mujer.";
  const normalText = "Esta ruta se eligió al entrar y las demás quedan cerradas para mantener la progresión correcta.";
  box.innerHTML = `<div><b>${state.isAdmin ? "Revisión administradora" : "Ruta activa"}: ${routeLabel()}</b><span>${message || (state.isAdmin ? adminText : normalText)}</span></div><button id="route-family-open" type="button">${familyTitle}</button><small>${familyText}</small>`;
  document.getElementById("route-family-open").onclick = () => {
    if (!state.family?.open) toggleFamilyMode();
    else document.getElementById("family-game").scrollIntoView({ behavior: "smooth", block: "start" });
  };
  document.querySelectorAll(".levels button[data-level]").forEach(button => {
    const locked = state.lockedLevel || selectedLevel;
    button.disabled = !state.isAdmin && +button.dataset.level !== locked;
    button.title = button.disabled ? "Ruta cerrada para este estudiante" : state.isAdmin ? "Disponible para revisión" : "Ruta activa";
  });
}
...
function renderEntrepreneurTabs() {
  ...
  tabs.querySelectorAll("[data-track]").forEach(button => button.disabled = selectedLevel !== 4);
...
function renderMission() {
  ...
  const critical = criticalEventFor(selectedLevel, selectedEntrepreneurTrack, activeMission, activeQuestion);
  const showCritical = shouldShowCritical(activeMission, activeQuestion) && activeCriticalAnswers[activeQuestion] === undefined;
  const lines = characterLines(selectedLevel, activeMission.topic, activity.place, mainGuide, support, activity);
  const supportLine = showCritical ? critical.guideLine : quietSupportLine(selectedLevel, support);
  const selectedCritical = activeCriticalAnswers[activeQuestion];
  const criticalResult = selectedCritical !== undefined ? critical.options[selectedCritical] : null;
  const criticalHtml = showCritical
    ? renderCriticalEvent(critical, selectedCritical)
    : selectedCritical !== undefined
      ? `<div class="critical-summary"><b>Situación crítica resuelta</b><span>${criticalResult?.result || "Has vuelto al reto principal."}</span></div>`
      : "";
  document.getElementById("mission-content").innerHTML = `
    <div class="mission-body">
      ...
      <div class="mission-grid">
        <aside class="card">
          <div class="guide-duo live-guides">${mini(mainGuide, "lead", lines.guideLine)}${mini(support, "support", supportLine)}</div>
          ...
          <div class="interaction">
            ...
            <p><b>${support.name}</b>: ${supportLine}</p>
        ...
        <section>
          ...
          ${criticalHtml}
          ${showCritical ? "" : `<div class="card activity activity-${activity.type}">
          ...
          <div class="actions">
            ...
            <button id="next" ${showCritical || activeAnswers[activeQuestion] === undefined ? "disabled" : ""}>${activeQuestion === total - 1 ? "Calificar" : "Siguiente"}</button>
          </div>
        </section>
      </div>
    </div>`;
  ...
  document.querySelectorAll("[data-critical]").forEach(button => {
    button.onclick = () => {
      document.querySelectorAll("[data-critical]").forEach(item => item.classList.remove("selected"));
      button.classList.add("selected");
      activeCriticalAnswers[activeQuestion] = +button.dataset.critical;
...
function renderActivityControl(activity) {
  ...
  if (activity.type === "true") {
    return `<div class="activity-control true-control">${activity.options.map((option, index) => button(option, index, `<b>${String.fromCharCode(65 + index)}</b><span>${option.text}</span>`)).join("")}</div>`;
  }
...
function shouldShowCritical(mission, questionIndex) {
  return questionIndex > 0 && questionIndex % 5 === 0;
}

function criticalEventFor(level, track, mission, questionIndex) {
  const topic = topicClean(mission.topic).toLowerCase();
  const banks = {
    1: [
      {
        title: "La compra sorpresa",
        situation: "Aparece algo muy bonito justo antes de pagar. También querías guardar monedas para tu meta.",
        guideLine: "Alto un segundo: esto no es la pregunta, es una situación de vida real.",
        options: [
          { text: "Compro rápido porque me gusta", result: "Te diviertes un momento, pero tu meta queda más lejos.", tone: "risk" },
          { text: "Miro mi dinero y pregunto si puedo esperar", result: "Buena pausa: comparas deseo y meta antes de gastar.", tone: "good" },
          { text: "Escondo el recibo para que nadie pregunte", result: "Eso crea confusión. El dinero se aprende hablando claro.", tone: "bad" }
        ]
      },
      {
        title: "Cambio mal contado",
        situation: "Te devuelven cambio, pero no sabes si está bien.",
        guideLine: "Respira: contar otra vez también es aprender dinero.",
        options: [
          { text: "Lo guardo sin mirar", result: "Puedes perder dinero sin darte cuenta.", tone: "risk" },
          { text: "Cuento despacio y pido ayuda", result: "Perfecto: revisar evita errores.", tone: "good" },
          { text: "Me enfado y me voy", result: "La emoción tapa el aprendizaje.", tone: "bad" }
        ]
      }
    ],
    2: [
      {
        title: "Oferta con presión",
        situation: "Una app dice que la oferta termina en 3 minutos y tus amigos insisten en comprar.",
        guideLine: "Esto es un imprevisto: decide sin dejarte empujar.",
        options: [
          { text: "Compro antes de que termine", result: "La prisa puede esconder cargos o una compra innecesaria.", tone: "risk" },
          { text: "Reviso precio final, saldo y si lo necesito", result: "Decisión fuerte: controlas impulso, grupo y pantalla.", tone: "good" },
          { text: "Uso cualquier enlace para pagar", result: "Riesgo alto: puedes exponer datos o dinero.", tone: "bad" }
        ]
      },
      {
        title: "Suscripción olvidada",
        situation: "Te cobran una suscripción que casi no usas.",
        guideLine: "No es castigo: es una oportunidad para ordenar tu dinero.",
        options: [
          { text: "La dejo porque revisar da pereza", result: "El gasto pequeño repetido se vuelve grande.", tone: "risk" },
          { text: "La cancelo o la cambio si no aporta", result: "Bien: proteges tu mesada y tu meta.", tone: "good" },
          { text: "Pido otra tarjeta para no verlo", result: "Eso oculta el problema y puede crear deuda.", tone: "bad" }
        ]
      }
    ],
    3: [
      {
        title: "Gasto urgente en casa",
        situation: "Aparece un arreglo inesperado y también querías hacer una compra grande.",
        guideLine: "Situación crítica: decide cuidando liquidez y calma.",
        options: [
          { text: "Uso toda la reserva en la compra grande", result: "La reserva queda débil justo cuando más se necesita.", tone: "risk" },
          { text: "Priorizar arreglo, revisar reserva y aplazar compra", result: "Buena decisión: estabilidad antes que impulso.", tone: "good" },
          { text: "Pido deuda sin mirar plazo ni coste", result: "La urgencia puede salir muy cara.", tone: "bad" }
        ]
      },
      {
        title: "Mensaje del banco",
        situation: "Llega un mensaje pidiendo entrar a un enlace para desbloquear la cuenta.",
        guideLine: "Aquí manda la seguridad: no todo mensaje merece confianza.",
        options: [
          { text: "Entro rápido al enlace", result: "Riesgo alto de fraude o robo de datos.", tone: "bad" },
          { text: "Abro la app oficial o llamo al banco", result: "Correcto: verificas por canal seguro.", tone: "good" },
          { text: "Comparto el mensaje con más datos", result: "Puedes aumentar el riesgo para ti y otros.", tone: "risk" }
        ]
      }
    ],
    entrepreneur: [
      {
        title: "Cliente grande, pago lento",
        situation: "Un cliente importante quiere comprar mucho, pero pagará dentro de 90 días.",
        guideLine: "Situación crítica de negocio: venta no siempre significa caja.",
        options: [
          { text: "Acepto todo sin anticipo", result: "Puedes vender mucho y quedarte sin caja.", tone: "risk" },
          { text: "Negocio anticipo, calendario y límite de entrega", result: "Buena dirección: proteges caja, cliente y operación.", tone: "good" },
          { text: "Bajo el precio sin calcular margen", result: "Puedes crecer perdiendo dinero.", tone: "bad" }
        ]
      },
      {
        title: "Proveedor sube precios",
        situation: "Tu proveedor principal sube costes de golpe y tienes pedidos abiertos.",
        guideLine: "Imprevisto empresarial: margen, reputación y caja chocan.",
        options: [
          { text: "Ignoro el cambio y vendo igual", result: "El margen puede desaparecer.", tone: "risk" },
          { text: "Recalculo coste, margen y comunico a tiempo", result: "Buena gestión: orden antes que sorpresa.", tone: "good" },
          { text: "Culpo al cliente y corto la relación", result: "Puedes dañar reputación y ventas futuras.", tone: "bad" }
        ]
      }
    ],
    woman: [
      {
        title: "Te piden bajar demasiado el precio",
        situation: "Un cliente dice que tu trabajo es caro y te presiona para cobrar mucho menos.",
        guideLine: "Situación crítica de Mujer Emprendedora: precio justo también es autonomía.",
        options: [
          { text: "Acepto aunque no cubra mis costes", result: "Trabajar sin margen debilita el negocio y tu seguridad.", tone: "bad" },
          { text: "Explico valor, coste, alcance y condiciones", result: "Excelente: negocias con datos y proteges tu caja.", tone: "good" },
          { text: "Regalo extras para no perder el cliente", result: "Puede parecer amable, pero rompe tiempo y margen.", tone: "risk" }
        ]
      },
      {
        title: "Mezcla entre casa y negocio",
        situation: "La caja del negocio se usa para un gasto personal urgente y no queda registro.",
        guideLine: "Situación crítica: separar dinero protege independencia y claridad.",
        options: [
          { text: "Lo dejo así porque después me acuerdo", result: "Sin registro, la caja se vuelve invisible.", tone: "risk" },
          { text: "Registro el movimiento y planifico devolución", result: "Bien: mantienes control y transparencia.", tone: "good" },
          { text: "Saco más dinero para compensar el estrés", result: "Eso puede romper caja y confianza.", tone: "bad" }
        ]
      }
    ]
  };
  const key = level === 4 ? (track === "woman" ? "woman" : "entrepreneur") : level;
  const list = banks[key] || banks[1];
  const event = list[(mission.id + questionIndex) % list.length];
  return { ...event, context: `Conecta con el tema: ${topic}.` };
}

function renderCriticalEvent(event, selected) {
  return `<div class="critical-event">
    <div class="critical-head"><span>Situación crítica</span><h3>${event.title}</h3><p>${event.situation}</p><small>${event.context}</small></div>
    <div class="critical-options">${event.options.map((option, index) => `<button type="button" data-critical="${index}" class="critical-option ${option.tone} ${selected === index ? "selected" : ""}"><b>${index + 1}</b><span>${option.text}</span><small>${option.result}</small></button>`).join("")}</div>
  </div>`;
}

function quietSupportLine(level, support) {
  return {
    1: `Yo te acompaño: mira la respuesta más tranquila.`,
    2: `Yo te ayudo a revisar si hay presión o gasto escondido.`,
    3: `Yo reviso contigo si la decisión mantiene estabilidad.`,
    4: `Yo miro caja, cliente y riesgo antes de avanzar.`
  }[level] || `${support.name} te acompaña en el reto.`;
}
...
function recordAttempt(correct, score, passed, impact, insight) {
  ...
  const attempt = {
    ...
    criticalSituations: Object.entries(activeCriticalAnswers).map(([questionIndex, optionIndex]) => {
      const event = criticalEventFor(selectedLevel, selectedEntrepreneurTrack, activeMission, +questionIndex);
      const option = event.options[optionIndex];
      return {
        beforeActivity: +questionIndex + 1,
        title: event.title,
        situation: event.situation,
        selectedOption: option?.text || "",
        consequence: option?.result || ""
      };
    }),
